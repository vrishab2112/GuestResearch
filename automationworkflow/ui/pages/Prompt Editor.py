...
from prompts.loader import load_overrides, save_overrides, get_prompt
from agent2.prompts import NORTH_STAR_SYSTEM as A2_DEFAULT_SYSTEM, NORTH_STAR_USER_TEMPLATE as A2_DEFAULT_USER
from agent3.prompts import SYSTEM as A3_DEFAULT_SYSTEM, USER_TEMPLATE as A3_DEFAULT_USER
...
with c1:
    ...
    with st.expander("Show default (currently used if override empty)"):
        st.code(A2_DEFAULT_SYSTEM)
    st.caption("Effective (used now):")
    st.code(get_prompt("agent2.system", A2_DEFAULT_SYSTEM))
    st.subheader("Agent 2 – User Template")
    a2_user = st.text_area("agent2.user", value=data.get("agent2.user", ""), height=220)
    with st.expander("Show default (currently used if override empty)"):
        st.code(A2_DEFAULT_USER)
    st.caption("Effective (used now):")
    st.code(get_prompt("agent2.user", A2_DEFAULT_USER))
with c2:
    st.subheader("Agent 3 – System Prompt")
    a3_system = st.text_area("agent3.system", value=data.get("agent3.system", ""), height=220)
    with st.expander("Show default (currently used if override empty)"):
        st.code(A3_DEFAULT_SYSTEM)
    st.caption("Effective (used now):")
    st.code(get_prompt("agent3.system", A3_DEFAULT_SYSTEM))
    st.subheader("Agent 3 – User Template")
    a3_user = st.text_area("agent3.user", value=data.get("agent3.user", ""), height=220)
    with st.expander("Show default (currently used if override empty)"):
        st.code(A3_DEFAULT_USER)
    st.caption("Effective (used now):")
    st.code(get_prompt("agent3.user", A3_DEFAULT_USER))
